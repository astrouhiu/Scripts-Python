...
def grade_Troianos(sistema, pos, apmax, apmin, apmed, incp, capomp, omegap, capmp):

	fator = 5

	if sistema == 'Kepler-9' and pos == 'i':
		fator = 3000

	sigma_a = apmax - apmin
	delta_a = fator*sigma_a
	...
	for i in range(100):
		...
		for j in range(100):
			...
			data.write("{:5d}  {:17.15f}  {:7.5f}  {:5.3f}  {:5.1f}  {:5.1f}  {:5.1f}\n".format(n, a, e, inc, capom, omega, capm))
			n = n + 1
			e = e + dive
...
def sistemas_reais(sistema):
	...
	os.system('mv /home/planeta9/Dropbox/Artigo_mestrado/Scripts/*SyMBA .')
	...
	os.system('cp /home/planeta9/Dropbox/Artigo_mestrado/Scripts/frequencia.dat .')
	...
	print("min(a2), min(a3), min(a4) = ", min(a2), min(a3), min(a4))
	print("med(a2), med(a3), med(a4) = ", np.mean(a2), np.mean(a3), np.mean(a4))
	print("max(a2), max(a3), max(a4) = ", max(a2), max(a3), max(a4))
	...
	for i in range(3):
		...
		grade_Troianos(sistema, pos[i], amax[i], amin[i], amed[i], incp[i], capomp[i], omegap[i], capmp[i])
	
	# Construindo diretórios de corridas com Troianos
	# -------------------------------------------------------------------------------------------------------------------------------
	
	os.system('mv /home/planeta9/Dropbox/Artigo_mestrado/Scripts/*rmvs .')
